/* NoodleBomb welcome offer popup — Klaviyo email capture + first-order code.
   WELCOME10 and the NoodleBomb Email List were live-verified before enabling.
...
(function () {
  var CFG = {
    ...
    sub: 'Join the Flavor Club for sauce drops, restock alerts, and recipes. Unsubscribe anytime.',
  ...
  function subscribeEmail(email) {
    return new Promise(function (resolve, reject) {
      ...
      (function waitForCapture() {
        if (typeof window.NBKlaviyoSubscribe === 'function') {
          window.NBKlaviyoSubscribe(email, 'NoodleBomb website: welcome-popup').then(resolve, reject);
          return;
        }
        if (attempts++ < 30) window.setTimeout(waitForCapture, 100);
        else reject(new Error('Klaviyo signup is not ready'));
      })();
    });
  }
  ...
  function show() {
    ...
    wrap.innerHTML =
      ...
      '<div class="nb-welcome-card">' +
        ...
        '<form class="nb-welcome-form" data-lead-form="welcome-popup">' +
          ...
          '<p class="nb-welcome-status" role="alert" aria-live="polite"></p>' +
        '</form>' +
    ...
    css.textContent =
      ...
      '.nb-welcome-form{display:flex;gap:8px;flex-wrap:wrap;}' +
      ...
      '.nb-welcome-status{flex-basis:100%;min-height:18px;margin:0;color:#E84A3A;font:600 12px/1.4 Inter,sans-serif;}' +
      ...
      '@media(max-width:480px){.nb-welcome-card{padding:30px 22px 22px}.nb-welcome-form input[type=email],.nb-welcome-form button{flex-basis:100%;width:100%}}';
    ...
    wrap.querySelector('.nb-welcome-form').addEventListener('submit', function (e) {
      ...
      var email = String(form.querySelector('input[type=email]').value || '').trim();
      var status = form.querySelector('.nb-welcome-status');
      btn.disabled = true;
