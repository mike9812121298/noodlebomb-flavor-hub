...
const hasBothShoyu = (items) => (items || []).some((i) => i.slug === 'shoyu' && (Number(i.qty) || 0) > 0) && (items || []).some((i) => i.slug === 'shoyuspicy' && (Number(i.qty) || 0) > 0);
...
function CartPage() {
  ...
  const freeShipping = bottleCount >= 5 && hasBothShoyu(items);
  const remaining = freeShipping ? 0 : Math.max(5 - bottleCount, 0);
  const progress = freeShipping ? 100 : Math.min((bottleCount / 5) * 100, 100);
    ...
    <>
      ...
      <div className="layout">
        ...
        <div>
          ...
          {freeShipping ? (
            <div className="card ship-bar unlocked">
              ...
              <div>
                ...
                <div style={{ color: 'var(--ink-40)', fontSize: 12, marginTop: 2 }}>All 5 bottles incl. both Shoyu — shipping's on us.</div>
              </div>
            </div>
          ) : (
            <div className="card ship-bar">
              <div className="row">
                <span style={{ color: 'var(--ink-60)' }}>{remaining > 0 ? <><span className="accent">{remaining} more bottle{remaining === 1 ? '' : 's'}</span> + both Shoyu for <span style={{ color: 'var(--ink)' }}>FREE US shipping</span></> : <>Add <span className="accent">both Shoyu</span> for <span style={{ color: 'var(--ink)' }}>FREE US shipping</span></>} · else $3.50 flat</span>
                <Truck />
              </div>
          ...
          {/* Power up your cart — Fire Dust one-tap upsell */}
          {!items.some((i) => i.slug === FIRE_DUST.slug) && (
        ...
        <div className="card summary">
          ...
          <div className="trust">
            ...
            <div className="trust-row"><Truck /> $3.50 flat US shipping · FREE on all 5 bottles (both Shoyu)</div>
