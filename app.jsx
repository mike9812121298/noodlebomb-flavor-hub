...
const buildWorldGlobeDots = () => {
  const dots = [];
  for (let lat = -56; lat <= 80; lat += 0.56) {
    for (let lng = -180; lng <= 180; lng += 0.62) {
...
function LiveOrderGlobeCanvas({ locations, latestLocation, isStoreLayer, spin, zoom, isDragging, onPointSelect }) {
  ...
  useEffect(() => {
    ...
    const render = (time = 0) => {
      ...
      const dpr = orderGlobeClamp(window.devicePixelRatio || 1, 1, 2.75);
      ...
      landDots.forEach((dot) => {
        ...
        if (!projected.visible || projected.alpha <= 0.04) return;
  ...
  }, [locations, latestKey, isStoreLayer, spin?.x, spin?.y, zoom]);
  ...
  const handlePointerMove = (event) => {
    ...
    const hit = projectedPinsRef.current
      .map((pin) => ({ ...pin, distance: Math.hypot(pin.x - x, pin.y - y) }))
      .filter((pin) => pin.distance <= pin.radius)
                ...
                <div
                  ...
                  <div style={{ marginTop: 32, paddingTop: 20, borderTop: '1px solid var(--line)' }}>
                    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 16 }}>
                      <div className="serif" style={{ fontSize: 22, fontStyle: 'normal' }}>{f.price}</div>
...
function BombGlobe({ mode, autoSpin, spinSpeed, shippedRef, arcsRef, focusRef, searchRef, storesRef }) {
  ...
  useEffect(() => {
    ...
    const globe = window.createGlobe(canvas, {
      ...
      dark: tdef.dark, diffuse: tdef.diffuse, mapSamples: 22000, mapBrightness: tdef.mapBrightness,
...
function OrderMapSection() {
  ...
  useEffect(() => {
    ...
    const loadLive = () => loadJson('/.netlify/functions/order-map', 4800)
      .then((data) => { if (data) applyCityData(data, 'live'); });
    const load = async () => {
...
function BuildBundle() {
  ...
  const statusLine = totalBottles === 0
    ...
    : trioSets > 0
      ? `${trioSets} discounted Trio ${trioSets === 1 ? 'combo' : 'combos'} banked. ${bottlesToNextTrio === 3 ? 'Add 3 more bottles to unlock another.' : `Add ${bottlesToNextTrio} more bottle${bottlesToNextTrio === 1 ? '' : 's'} to unlock the next one.`}`
      : `Add ${bottlesToNextTrio} more bottle${bottlesToNextTrio === 1 ? '' : 's'} to unlock Trio savings.`;
  const comboLine = trioSets > 0
    ? `${trioSets} Trio ${trioSets === 1 ? 'combo' : 'combos'} = ${trioBottles} bottles at bundle price`
    : 'Any 3 bottles unlock the Trio price';
      ...
      <div className="bundle-shell">
        <Reveal>
          <div className="bundle-head">
            ...
            <p>
              Tap + to stack bottles like a loadout. Any 3 bottles unlock Trio savings, no matching flavors required, and every bottle pushes the power meter higher.
            </p>
